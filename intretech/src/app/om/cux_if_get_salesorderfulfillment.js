...
function(record, search, runtime, dao, utils) {
	 ...
	 * @param requestParams.filterList - 混合二维数组，偶数元素是长度2或3的一维数据（谓词要素），奇数元素是逻辑运算符 requestParams.filterList = [ [ 'name', 'is', 'EAI.TEST.190422A01-cBOM' ] ];
	...
	function doGet(requestParams) {
		...
		try {
			...
			return {
				"resultSet" : dao.getSalesOrderFulfillment(filterList)
			};
		} catch (e) {
			log.debug({
