...
define([],
	...
	function padZero(param) {
		if (String(param).length == 1)
			return String("0").concat(String(param));
	...
	function formatDate(date,format){
		...
		return format.replace(/YYYY|MM{0,3}|DD?o?|dddd|HH?|mm|ss/g, 
				function (tag) {
			if (tag == "YYYY") return date.getFullYear();
			if (tag == "M") return date.getMonth()+1;
			if (tag == "MM") return padZero(date.getMonth()+1);
			...
			if (tag == "H") return date.getHours();
			if (tag == "HH") return padZero(date.getHours());
			if (tag == "mm") return padZero(date.getMinutes());
			if (tag == "ss") return padZero(date.getSeconds());}
			);
	}
	
	function nsDate(dateString){
		var dateObject = null;
		dateObject.YYYYMMDD = function formatDate(date,format){
			var days = dateNames().days;
			var months = dateNames().months;
			var abbreviated_months = dateNames().abbreviated_months;
			return format.replace(/YYYY|MM{0,3}|DD?o?|dddd|HH?|mm|ss/g, 
