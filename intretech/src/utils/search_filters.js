...
define([ 'N/search' ],
		...
		function(search) {
			...
			function setJoinFilters(searchType, joinFilterValues) {
				...
				case search.Type.TRANSACTION:
					return setStatusAndTimeStampFilters(filterValues.lastmodifieddate).concate(
							createFilters(_transactionDefaultSearchJoinFilters, joinFilterValues));
					break;
			...
			/**
			 * A single search.Filter object, an array of search.Filter objects, a search filter expression, or an array of search filter expressions. Cannot
			 * mixed with Filter Object and filter expression in an array.
			 */
			...
			function setMainFilterExpressions(searchType, joinFilterExpressions) {
				...
				//bom
				var _bomDefaultSearchFilterExpressions = [ [ "isinactive", "isnot", "T" ], "AND", [ "subsidiary", "is", "1" ], "AND",
			...
			function setJoinFilterExpressions(searchType, newJoinFilterExpressions) {
				...
				//bom
				//, "AND", [ "includechildren", "is", "T" ], "AND", [ [ "revision.name", "contains", "v1" ], "or", [ "revision.name", "contains", "v2" ] ]
				var _bomDefaultSearchJoinFilterExpressions = [ [ "isinactive", "isnot", "T" ], "AND", [ "subsidiary", "is", "1" ], "AND",
				...
				switch (searchType) {
				case search.Type.ITEM:
					return createJoinFilterExpressions(_itemSearchFilterNames, newJoinFilterExpressions);
					break;

				case search.Type.BOM:
					return createJoinFilterExpressions(_bomDefaultSearchJoinFilterExpressions, newJoinFilterExpressions);
					break;

				case search.Type.TRANSACTION:
					return createJoinFilterExpressions(_transactionDefaultSearchJoinFilterExpressions, newJoinFilterExpressions);
					break;
