...
public class Fields extends LinkedHashMap<String, String> {
	...
	public Fields(Bom bom) {
		super();
		put(INTERNAL_ID, bom.getInternalId());
		put(EXTERNAL_ID, bom.getExternalId());
		put(NAME, bom.getName());
		put(MEMO, bom.getMemo());
		put(IS_INACTIVE, String.valueOf(bom.getIsInactive()));
		put(DATE_CREATED, bom.getCreatedDate().getTime().toString());
	}
	...
	private static String getAddressBookLabel(Customer customer) {
		final int i = 0;
		if (customer.getAddressbookList() == null || customer.getAddressbookList() == null || customer.getAddressbookList().getAddressbook(i) == null) {
			return null;
		}
	...
	private static String getVendorAddressBookLabel(Vendor vendor) {
		final int i = 0;
		if (vendor.getAddressbookList() == null || vendor.getAddressbookList() == null || vendor.getAddressbookList().getAddressbook(i) == null) {
			return null;
		}
		return vendor.getAddressbookList().getAddressbook(i).getLabel();
	}
}
