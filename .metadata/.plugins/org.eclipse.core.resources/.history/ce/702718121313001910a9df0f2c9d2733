...
function(record, render, search) {
	function onRequest(params) {
		...
		g_print_header2 += '<TABLE border=0 cellpadding=0 cellspacing=0 width=' + g_table_width + '>';
		g_print_header2 += '<TR><th colspan=' + g_table_column + ' class=subheader1 align=center style="margin-top: 0">物料主数据报表</th></TR>';
		g_print_header2 += '<TR><TD colspan=' + g_table_column + ' class=subheader2>当前日期：P_SYSDATE_TIME</TD></TR>';
		...
		g_table_title += '                 <td class="tableheader" nowrap width="200">采购单位</td>';
		g_table_title += '                 <td class="tableheader" nowrap width="200">存储描述</td>';
		g_table_title += '     <td class="tableheader" nowrap width="200">存储单位</td>';
		...
		g_print_header2 = g_print_header2.replace('P_SYSDATE_TIME', sysdateTime);
		xmlStr = g_print_header0 + g_print_header1 + g_print_header2 + g_table_title;
		...
		try {
			...
			for (var i = 0; i < itemRec.length; i++) {
				...
				l_table_body = l_table_body.replace('TEXT02', itemRec[i].getValue('itemid')); //字符1
				l_table_body = l_table_body.replace('TEXT03', itemRec[i].getValue('purchasedescription')); //字符2
				l_table_body = l_table_body.replace('TEXT04', itemRec[i].getValue('purchaseunit')); //字符3
				l_table_body = l_table_body.replace('TEXT05', itemRec[i].getValue('stockdescription')); //数量1
				l_table_body = l_table_body.replace('TEXT06', itemRec[i].getValue('stockunit')); //单价1
				l_table_body = l_table_body.replace('TEXT07', itemRec[i].getValue('cost')); //金额1
				xmlStr += l_table_body;
			}*/
			//不支持以下搜索
			/*var itemRec = search.create({
			...
			itemRec.each(function(result) {
				...
				var l_table_body = g_table_body;
				l_table_body = l_table_body.replace('TEXT01', l_row_num);
				l_table_body = l_table_body.replace('TEXT02', result.getValue('itemid')); //字符1
				l_table_body = l_table_body.replace('TEXT03', result.getValue('purchasedescription')); //字符2
				l_table_body = l_table_body.replace('TEXT04', result.getValue('purchaseunit')); //字符3
				l_table_body = l_table_body.replace('TEXT05', result.getValue('stockdescription')); //数量1
				l_table_body = l_table_body.replace('TEXT06', result.getValue('stockunit')); //单价1
				l_table_body = l_table_body.replace('TEXT07', result.getValue('cost')); //金额1
				xmlStr += l_table_body;
			});
			*/
			//推荐:分页加载
