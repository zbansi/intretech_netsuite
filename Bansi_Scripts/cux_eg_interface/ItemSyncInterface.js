...
function(record, serverWidget, search, file, runtime, config, format) {
	...
	function onRequest(context) {
		...
		//NS UI:YYYY/MM/DD hh:mm:ss 2019/01/02 21:10:24 (must match yy/MM/dd H:mm)		
		//eclipse调用： date value 1/2/2019 21:11:14 (must match M/d/yy H:mm)
		...
		if (dateArray[0].length > 2) {// YYYY/MM/DD hh:mm:ss 转成 M/d/yy H:mm
			yy = dateArray[0].slice(2, 4);
			mon = dateArray[1];
			da = dateArray[2];
		} else if (dateArray[2].length > 2) {// M/d/yyyy hh:mm:ss 转成 M/d/yy H:mm
